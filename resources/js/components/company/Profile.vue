        ...
        <v-col>
          <v-form ref="form" lazy-validation>
            ...
            <v-row>
             ...
             <v-row>
              <v-col cols="12">
                <UserSelectMultiple :usersSelected.sync="personal" :usersRemoved.sync="personalRemoved" :users="personalSelect" label="Personal" />
              </v-col>
            </v-row>
...
export default {
  ...
  computed: {
    ...
    ownerSelect(){
      return this.usersSelect.filter(item => item.role.slug == 'agency' || false)
    },
    personalSelect(){
      return this.usersSelect.filter(item => item.role.slug != 'agency' &&  (!this.owner || item.id != this.owner.id))
    }
  },
  methods: {
    ...mapActions({
      getItem: "company/getItem",
      getUsers: "company/getItemsGeneric",
        ...
        endpoint: "users-select",
            ...
            .then(response => {
              if (response.data.data.length)
                this.usersSelect = response.data.data;
            })
            .catch(e => {
              console.log(e);
            });
