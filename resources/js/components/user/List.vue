  ...
  <div>
    <v-row>
      ...
      <v-col class="d-flex justify-end">
        <v-btn
          rounded
          color="primary"
          @click="$router.push({name: 'representative_edit_profile', params: { id: 'new' }})"
        >
    ...
    <v-row v-if="items.length">
      <v-col cols="12" md="4" class="pa-1" v-for="(item, index) in items" :key="index">
        <ListUserCard :item="item" role="representative" v-on:remove="remove" />
      </v-col>
    </v-row>
    <v-row v-else>
      <v-col cols="12">
...
export default {
  ...
  computed: {
    ...mapFields("representative", ["page", "items", "loadingList", "perPage", "pages", "search"])
  },
  methods: {
    ...mapActions({
      removeUser: "user/remove",
      getItemsAction: "representative/getItems"
    }),
    getItems(){
      this.getItemsAction({
        endpoint: "users"
