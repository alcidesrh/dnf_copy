            ...
            <tbody>
              <tr>
                <td style="border-bottom:1px solid #f6f6f6;">
                  <h1 style="font-size:14px; font-family:arial; margin:0px; font-weight:bold;">Hola {{ $user->first_name }},</h1>
                  <p style="margin-top:0px; color:#777777;">
                    <br/>
                    Usted a sido añadido como un {{ $user->role->slug == 'broker'?'Representative':($user->role == 'elite'?'Elite Representative':ucfirst($user->role))}} con {{$user->company->name}} en {{config('insura.name')}}.
              ...
              <tr>
                <td style="padding:10px 0 30px 0;">
                  ...
                  <center>
                    {{-- {{ action('Auth\PasswordController@getActivate', array($token)) }} --}}
                  <a href="{{config('app.url')}}/token/{{$token}}" style="display: inline-block; padding: 11px 30px; margin: 20px 0px 30px; font-size: 15px; color: #fff; background: #00c0c8; border-radius: 60px; text-decoration:none;">Activar cuenta</a>
                  </center>
                  <p style="color:#444444;">company@email.com</p>
                </td>
              </tr>
           
              <tr>
                <td  style="border-top:1px solid #f6f6f6; color:#bbbbbb; font-size:10px; padding-top:20px;">
                  {{ trans('emails.report.welcome') }} <a href="mailto://company@email.com">ownercompany@email.com</a>
                </td>
              </tr>
            </tbody>
          </table>
